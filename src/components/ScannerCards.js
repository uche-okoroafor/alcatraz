...
const ScannerCards = (props) => {
    const { setLoading, handleCardClick, runningScanner, newAddedSetup, activeRunningScanner, setActiveRunningScanner, newSignals } = props;
    ...
    const updateRunningStrategies = () => {
        ...
        const handleCheck = (element) => {
            ...
            if (!activeRunningScanner[element._id]) {
                tempObj[element._id] = {
                    _id: element._id,
            ...
            } else if (activeRunningScanner[element._id]?.failedRunCount === 1) {
    ...
    const getStatusIcon = (scanner) => {
        ...
        } else if (scanner.is_active && activeRunningScanner[scanner._id]?.isRunning) {
            return "styles_dot__p5bay styles_online__X4EkV"
        } else if (scanner.is_active && activeRunningScanner[scanner._id]?.isRunning === undefined) {
    ...
    const runError = (scanner) => {
        if (activeRunningScanner?.errors[scanner._id]) {
            console.log(activeRunningScanner.errors[scanner._id])
                        ...
                        <Card className="strategy-card" onClick={() => handleCardClick(scanner)}>
                            ...
                            <CardContent sx={{ position: 'relative' }}>
                                <div className='d-flex justify-content-between'>
                                    <div >
                                        <Typography><strong>Scanner:</strong> {scanner.scanner_type}</Typography>
                                ...
                                {runningScanner.includes(scanner._id) && (
                                    <div className={`progress ${runningScanner.includes(scanner._id) ? '' : 'progress-hidden'}`}>
                                        <div
